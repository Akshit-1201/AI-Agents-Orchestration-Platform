...
> Phase 1 + 1.5 are complete; later phases map directly to the challenge's success criteria.
...
| External channel (WhatsApp/Telegram/Slack), ≥1 agent reachable | 5 |
...
- `backend/runtime/`: `compiler.py` (DB workflow → `StateGraph`), `nodes.py` (custom agent
  ...
  `rag.py` + `ingest.py` (Chroma vector DB), `providers.py` (Gemini→Ollama fallback + fake),
...
- Notes: used a **custom agent loop** (not `create_react_agent`) for fallback + instrumentation
  control; **LangGraph checkpointer deferred to Phase 5** (resume/HITL). See `PHASE2_PLAN.md`.
...
change. 32 tests green. (Deferred by design: checkpointer→P5, async/streaming→P3,
schedules + summary-memory→P5/6.)
...
### Phase 4 — Frontend (Next.js + React Flow + shadcn/ui)  ← NEXT
Agent CRUD + **full config UI** (channels, schedules, memory, skills, rules, guardrails, limits);
workflow builder with conditions + feedback loops; live monitor (logs, **inter-agent messages**,
token/cost) over the WebSocket; runs list; **message history visible**.

### Phase 5 — External Channel Integration
...
table maps chat↔run; uses `messages.direction/status/external_id` for reliability.
...
- **LLM routing**: Gemini 2.5 Flash → Ollama/Qwen fallback with logged retries.
